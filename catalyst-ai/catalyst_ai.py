...
FIXES APPLIED:
  1. pandas 2.x compat (ffill/bfill instead of fillna method=)
  2. FastAPI lifespan instead of deprecated @on_event("startup")
  3. Unified DB path (trading_performance.db)
  4. Integrated Daily95Optimiser into the main app
  5. Removed unused scipy import
  6. Proper WAT (Africa/Lagos) timezone via pytz
  7. Safe resample() with try/except for StructureAnalyzer
  8. APScheduler for signal_loop instead of bare asyncio.create_task
  9. Enhanced format() with volume_trend
 10. More sensitive volume_profile threshold
"""
...
class AutoFixer:
    ...
    def fix_df(df: pd.DataFrame) -> pd.DataFrame:
        """Fix common DataFrame issues (pandas 2.x compatible)."""
        df = df.copy()
    ...
    def fix_indicators(ind: dict) -> dict:
        defaults = {
            ...
            "adx": 25, "bb_width": 0.1, "bb_width_prev": 0.1,
            "atr": 0.001, "ema50": 0, "ema200": 0,
            "current_price": 0, "volatility": 0.5,
            "volume_spike": False, "volume_trend": "normal",
            "momentum": 0.0, "engulfing": False, "engulfing_dir": "none",
            "rejection": False, "rejection_dir": "none", "vol_quality": False,
        }
...
class Indicators:
    ...
    def stoch(high, low, close, k=14, d=3):
        ...
        for i in range(d):
            if len(close) > i + k:
                ...
                if hh_ != ll_:
                    dvals.append(100 * (close[-(1 + i)] - ll_) / (hh_ - ll_))
    ...
    def adx(high, low, close, p=14):
        ...
        atr_s = pd.Series(tr).ewm(alpha=1 / p, adjust=False).mean()
        pd_ = 100 * pd.Series(p_dm).ewm(alpha=1 / p, adjust=False).mean() / atr_s
        md_ = 100 * pd.Series(m_dm).ewm(alpha=1 / p, adjust=False).mean() / atr_s
    ...
    def ema(prices, p):
        ...
        return float(pd.Series(prices).ewm(span=p, adjust=False).mean().iloc[-1])
    ...
    def atr(high, low, close, p=14):
        ...
        atr_v = pd.Series(tr).ewm(alpha=1 / p, adjust=False).mean().iloc[-1]
        return float(atr_v) if not pd.isna(atr_v) else 0.001
    ...
    def rejection(df):
        ...
        if uw > 2 * body and lw < 0.3 * rng:
            return True, "bearish"
        if lw > 2 * body and uw < 0.3 * rng:
            return True, "bullish"
        return False, "none"
    ...
    def volume_profile(df):
        ...
        if avg <= 0:
            return False
        return vol[-1] > avg * 1.5  # FIX: lowered from 1.8 for more sensitivity
    ...
    def calc_all(data):
        ...
        avg_vol = (
            np.mean(data["volume"].values[-20:-1])
            if len(data["volume"]) >= 21
            else np.mean(data["volume"])
        )
        vol_spike = data["volume"].iloc[-1] > avg_vol * 1.8 if avg_vol > 0 else False
        if len(data["volume"]) >= 6:
            ra = np.mean(data["volume"].values[-3:])
            pa = np.mean(data["volume"].values[-6:-3])
            vol_trend = (
                "increasing"
                if ra > pa * 1.1
                else ("decreasing" if ra < pa * 0.9 else "normal")
            )
        else:
            vol_trend = "normal"
        momentum = (
            (close[-1] - close[-10]) / close[-10] * 100
            if len(close) >= 10
            else 0.0
        )
        ...
        return AutoFixer.fix_indicators(
            {
                ...
                "adx": adx_v, "bb_width": bb_w, "bb_width_prev": bb_p,
                "ema50": ema50, "ema200": ema200, "atr": atr_v,
                "volatility": bb_w * 10,
                "volume_spike": vol_spike, "volume_trend": vol_trend,
                "current_price": price, "momentum": momentum,
                "engulfing": eng, "engulfing_dir": edir,
    ...
    def analyze(data):
        ...
        if len(h) >= 20:
            ...
            for i in range(5, len(h) - 5):
                if all(h[i] >= h[i - j] for j in range(1, 6)) and all(
                    h[i] >= h[i + j] for j in range(1, 6)
                ):
                    sh.append(h[i])
                if all(l[i] <= l[i - j] for j in range(1, 6)) and all(
                    l[i] <= l[i + j] for j in range(1, 6)
                ):
                    sl.append(l[i])
            if len(sh) >= 2 and len(sl) >= 2:
                if sh[-1] > sh[-2] and sl[-1] > sl[-2]:
                    ...
                    trend = "bearish"
            if c[-1] > np.max(h[-10:-1]):
                bos = True
            elif c[-1] < np.min(l[-10:-1]):
                bos = True
            if trend == "bullish" and c[-1] < np.min(l[-5:-1]):
                choch = True
            elif trend == "bearish" and c[-1] > np.max(h[-5:-1]):
                choch = True
            # multi-tf alignment (safe resample with try/except)
            if len(data) >= 40:
                try:
                    higher = (
                        data.resample("5min")
                    ...
                    pass  # resample only works with DatetimeIndex
        return {
            "trend": trend,
            "bos_confirmed": bos,
            "choch_confirmed": choch,
            "multi_tf_aligned": mlt,
        }
...
class Zones:
    ...
    def detect(data):
        ...
        if len(data) >= 3:
            lr = hi - lo
            pr = data["high"].iloc[-2] - data["low"].iloc[-2]
            if pr > 0 and lr > 2 * pr:
                ob = True
        fvg_active = False
        if len(data) >= 3:
            c = data.iloc[-1]
            p2 = data.iloc[-3]
            if c["low"] > p2["high"] or c["high"] < p2["low"]:
                fvg_active = True
        return {
            "at_supply": at_sup,
            "at_demand": at_dem,
...
class Liquidity:
    ...
    def analyze(data):
        data = AutoFixer.fix_df(data)
        h = data["high"].values[-10:]
        l = data["low"].values[-10:]
        sweep = False
        stype = "none"
        if len(h) >= 5:
            rh = max(h[:-1])
            rl = min(l[:-1])
            if h[-1] > rh and data["close"].iloc[-1] < rh:
                sweep = True
                stype = "buy_side"
            elif l[-1] < rl and data["close"].iloc[-1] > rl:
                sweep = True
                stype = "sell_side"
        build = (len(h) >= 3 and (max(h) - min(h)) < np.mean(h) * 0.001) or (
            len(l) >= 3 and (max(l) - min(l)) < np.mean(l) * 0.001
        )
...
class UltraFilter:
    ...
    def check(self, ind, struc, liq, zones):
        ...
        if struc["trend"] != "neutral":
            s1 += 40
        if struc["bos_confirmed"]:
            s1 += 40
        if struc["multi_tf_aligned"]:
            s1 += 20
        ...
        if ind["rsi"] > 75 or ind["rsi"] < 25:
            ...
            s2 += 10
        if abs(ind["stoch_k"] - ind["stoch_d"]) < 3 and (
            ind["stoch_k"] > 80 or ind["stoch_k"] < 20
        ):
            s2 += 30
        if (ind["ema50"] > ind["ema200"]) if struc["trend"] == "bullish" else (
...
# ============================================================
# 5. PERFORMANCE TRACKER & DAILY IMPROVER (integrated)
# ============================================================
class PerfTracker:
    ...
    def record(self, sig):
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT OR IGNORE INTO trades (signal_id,symbol,direction,timeframe,entry_time,feature_scores) VALUES (?,?,?,?,?,?)",
            (
                sig["signal_id"],
                sig["symbol"],
                sig["direction"],
                sig["timeframe"],
                sig["entry_time"].isoformat(),
    ...
    def recent_win_rate(self, days=7):
        ...
        cur.execute(
            "SELECT outcome FROM trades WHERE outcome!='pending' AND entry_time>=?",
            (cutoff.isoformat(),),
        )
        rows = cur.fetchall()
        conn.close()
...
class Daily95Optimiser:
    """
    Integrated daily optimiser that adjusts filter thresholds
    based on recent win rate and per-feature performance.
    Persists config to JSON so changes survive restarts.
    """

    def __init__(self, filter_obj: UltraFilter, config_path="config/trade_config_95.json"):
    ...
    def run(self):
        ...
        if wr < 0.95:
            ...
            # Above 95% for a long time - relax slightly for more signals
        ...
        logger.info(
            f"Daily95Optimiser: wr={wr:.2%}, "
            f"min_overall={self.filter.min_overall}, "
            f"min_confluences={self.filter.min_confluences}"
        )
        ...
        if len(rows) < 20:
            logger.info("Daily95Optimiser: not enough data for per-feature tuning (<20)")
            self._save_config()
            return
        ...
        for feature, values in feature_stats.items():
            ...
            for th in range(60, 100, 5):
                ...
                if len(pass_idx) > 5:
                    wr_feature = sum(outcomes[i] for i in pass_idx) / len(pass_idx)
                    if wr_feature >= 0.95:
        ...
        self._save_config()
        logger.info(
            f"Daily95Optimiser: completed. thresholds={self.filter.thresholds}"
        )
...
class SignalGenerator:
    ...
    def _direction(self, ind, struc, liq, zones):
        buy, sell = 0, 0
        if struc["trend"] == "bearish":
            sell += 35
        elif struc["trend"] == "bullish":
            buy += 35
        if liq["sweep_type"] == "buy_side":
            sell += 30
        elif liq["sweep_type"] == "sell_side":
            buy += 30
        if zones["at_supply"]:
            sell += 30
        if zones["at_demand"]:
            buy += 30
        if ind["engulfing_dir"] == "bearish":
            sell += 20
        elif ind["engulfing_dir"] == "bullish":
            buy += 20
        if ind["rejection_dir"] == "bearish":
            sell += 15
        elif ind["rejection_dir"] == "bullish":
            buy += 15
    ...
    async def get_data(self, symbol, tf):
        ...
        freq_map = {"1m": "1min", "2m": "2min", "3m": "3min", "5m": "5min"}
    ...
    async def generate(self, symbol, tf):
        try:
            ...
            entry = now_wat + timedelta(minutes=3)
            ...
            for i, (m, t) in enumerate(zip(mults, times)):
                ...
                if amt > 3.0:
                    ...
                    m = round(amt / 1.0, 1)
                mart.append(
                    {"level": f"M{i+1}", "multiplier": m, "amount": amt, "entry_time": t}
                )
    ...
    def format(self, sig):
        ...
        ml = [
            f"{m['level']} | {m['multiplier']}x | ${m['amount']} | Entry: {m['entry_time'].strftime('%H:%M WAT')}"
            for m in sig["martingale"]
        ]
        mart = "\n".join(ml)
        ind = sig["indicators"]
        struc = sig["structure"]
        ...
        liq_str = (
            ...
            else ("Building" if liq["building"] else "No Sweep")
        )
        # FIX: use volume_trend in output
        vol_str = (
            "High (Spike)"
            if ind["volume_spike"]
            else (
                ...
                else ("Falling" if ind["volume_trend"] == "decreasing" else "Normal")
            )
        )
        zp = []
                            ...
                            "martingale": [
                                {
                                    ...
                                    "amount": m["amount"],
                                    "entry_time": m["entry_time"].strftime("%H:%M WAT"),
                                }
                                for m in sig["martingale"]
                            ],
...
async def lifespan(app):
    ...
    scheduler.start()
    logger.info("CATALYST AI started - scheduler active")
...
async def api_outcome(signal_id: str, outcome: str):
    """Update trade outcome for performance tracking."""
    if outcome not in ("win", "loss", "breakeven"):
        return {"error": "outcome must be win/loss/breakeven"}
    gen.tracker.update(signal_id, outcome)
    return {"signal_id": signal_id, "outcome": outcome}
...
.header{background:linear-gradient(135deg,#0a0a2e,#1a1a4e);border-radius:16px;padding:20px 24px;display:flex;justify-content:space-between;align-items:center;border:1px solid #2a2a5a;margin-bottom:20px}
.logo{font-size:2em;font-weight:bold;letter-spacing:-1px}
.logo span{color:#00ff88}
.status-row{display:flex;align-items:center;gap:12px}
.status{color:#00ff88;background:rgba(0,255,136,0.1);padding:5px 15px;border-radius:20px;font-size:0.85em}
.stats-bar{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:20px}
.stat-card{background:linear-gradient(135deg,#0a0a2e,#1a1a4e);border-radius:12px;padding:14px 18px;border:1px solid #2a2a5a}
.stat-label{font-size:0.75em;color:#888;text-transform:uppercase;letter-spacing:1px}
.stat-value{font-size:1.4em;font-weight:bold;color:#00ff88;margin-top:4px}
.platform-selector{display:flex;gap:10px;margin-bottom:20px}
.platform-btn{padding:10px 25px;border-radius:25px;border:1px solid #2a2a5a;background:#0a0a2e;color:#e0e0e0;cursor:pointer;font-size:0.9em;transition:all 0.2s}
...
.signal-card{background:#1a1a3e;border-radius:12px;padding:20px;margin:15px 0;border-left:4px solid #00ff88;animation:slide 0.3s ease-out}
...
.meta{font-size:0.9em;color:#aaa;margin:4px 0}
.martingale{margin-top:10px;background:#111;padding:12px;border-radius:8px}
.martingale-header{color:#ffd700;font-weight:bold;margin-bottom:8px;font-size:0.9em}
.martingale-row{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #222;font-size:0.85em}
.signal-status{margin-top:10px;color:#00ff88;font-size:0.9em;font-weight:bold}
@keyframes slide{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
@media(max-width:600px){.app{padding:10px}.header{flex-direction:column;gap:10px}.stat-card{padding:10px}}
</style>
</head>
<body>
<div class="app">
  <div class="header">
    ...
    <div class="status-row">
      <span class="status" id="conn-status">🟢 Online</span>
    </div>
  </div>

  <div class="stats-bar" id="stats-bar">
    <div class="stat-card"><div class="stat-label">Signals</div><div class="stat-value" id="sig-count">0</div></div>
    <div class="stat-card"><div class="stat-label">Filter Min</div><div class="stat-value" id="filter-min">92%</div></div>
    <div class="stat-card"><div class="stat-label">Confluences</div><div class="stat-value" id="filter-conf">8</div></div>
    <div class="stat-card"><div class="stat-label">Clients</div><div class="stat-value" id="ws-clients">0</div></div>
  </div>

  <div class="platform-selector">
    <button class="platform-btn active" onclick="selectPlatform(this)">🎯 IQ Option</button>
    <button class="platform-btn" onclick="selectPlatform(this)">💼 Pocket Option</button>
  </div>

  <div style="color:#888;margin-bottom:15px;font-size:0.9em;">SMART MONEY &middot; AI-POWERED &middot; 94.3% FILTER &middot; WAT TIMEZONE</div>
...
function connect() {
  ...
  ws.onmessage = function(e) {
    ...
    if (d.type === 'new_signal') {
      signalCount++;
      document.getElementById('sig-count').textContent = signalCount;
      addSignalCard(d);
    }
  };
...
function addSignalCard(d) {
  var card = document.createElement('div');
  card.className = 'signal-card ' + d.direction.toLowerCase();

  var ml = '';
  if (d.martingale && d.martingale.length) {
    ml = '<div class="martingale"><div class="martingale-header">🛡️ MARTINGALE RECOVERY</div>';
    d.martingale.forEach(function(m) {
      ml += '<div class="martingale-row">' +
        '<span>' + m.level + '</span>' +
        '<span>' + m.multiplier + 'x</span>' +
        '<span>$' + m.amount + '</span>' +
        '<span>' + m.entry_time + ' WAT</span>' +
      '</div>';
    });
    ml += '</div>';
  }

  var dirEmoji = d.direction === 'SELL' ? '🔴' : '🟢';
  card.innerHTML =
    '<div class="card-header">' +
      '<div class="pair">🎫 ' + d.symbol + '</div>' +
      '<div class="conf">🎯 ' + d.confidence.toFixed(1) + '%</div>' +
    '</div>' +
    '<div class="direction ' + d.direction.toLowerCase() + '">' + dirEmoji + ' ' + d.direction + '</div>' +
    '<div class="meta">⏳ ' + d.timeframe + ' (OTC) | ➡️ Entry: ' + d.entry_time + '</div>' +
    ml +
    '<div class="signal-status">🎯 SIGNAL STATUS: HIGH PROBABILITY ONLY</div>';
...
function fetchHealth() {
  fetch('/health').then(function(r) { return r.json(); }).then(function(d) {
    document.getElementById('filter-min').textContent = d.filter_min_overall + '%';
    document.getElementById('filter-conf').textContent = d.filter_min_confluences;
    document.getElementById('ws-clients').textContent = d.ws_clients;
  }).catch(function() {});
}
...
// Load existing signals on page load
fetch('/api/signals').then(function(r) { return r.json(); }).then(function(d) {
  if (d.signals && d.signals.length) {
    signalCount = d.count;
    document.getElementById('sig-count').textContent = signalCount;
    d.signals.reverse().forEach(function(sig) {
      addSignalCard({
        type: 'new_signal',
        symbol: sig.symbol,
        direction: sig.direction,
        confidence: sig.confidence,
        entry_time: new Date(sig.entry_time).toLocaleTimeString('en-GB', {hour:'2-digit',minute:'2-digit'}) + ' WAT',
        timeframe: sig.timeframe,
        martingale: null
      });
    });
...
// Refresh health stats every 30s
