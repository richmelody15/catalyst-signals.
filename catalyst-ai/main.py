#!/usr/bin/env python3
"""
WINNING TURBO ENGINE - All Filters Mandatory, Aggressive Parameters
- All 9 mandatory conditions must pass, but thresholds relaxed for fast OTC Blitz.
- Dual-market data (IQ/PO + Yahoo Finance)
...
# Aggressive winning configuration (all mandatory)
CONFIG = {
    'ADX_MIN': 18,                # catch early trend building
    'VOLUME_MULT': 1.2,           # OTC volume can be subtle
    'RSI_OB': 62,                 # overbought threshold
    'RSI_OS': 38,                 # oversold threshold
    'MTF_MIN': 2,                 # still need 2/3 timeframes aligned
    'SR_PROXIMITY': 0.012,        # wider zone for fast markets
    'NEWS_AVOID': (12, 14),       # avoid US data hour
    'LIQUIDITY_WINDOW': 12,       # slightly shorter lookback
    'MOMENTUM_MIN': 0.00025,      # require a little less move
    'MARTINGALE': [('M1',1.5,1), ('M2',2.5,2), ('M3',4.0,3)],
    'MIN_CONFIDENCE': 75,         # take slightly more trades
    'DIVERGENCE_WINDOW': 10
}

# --- Dual-Market Data Feed ---
class DualMarketData:
    ...
    def get_otc_candles(self, platform, pair, timeframe_sec, count=200):
        """Get candles from live broker API (IQ Option or Pocket Option)."""
        try:
            if platform == 'iq' and self.iq:
                ...
                if not df.empty:
                    ...
                    return df
            elif platform == 'pocket' and self.po:
            ...
            print(f"OTC data error ({pair}): {e}")
        return pd.DataFrame()

    def get_forex_candles(self, pair, timeframe_sec, count=200):
        """Get real forex/commodity data from Yahoo Finance for cross-validation."""
        if not HAS_YF:
            return pd.DataFrame()
        ticker = self.forex_map.get(pair)
        if not ticker:
            return pd.DataFrame()
        try:
            if timeframe_sec <= 60:
                interval = '1m'
            elif timeframe_sec <= 300:
                interval = '5m'
            else:
                interval = '15m'
            period = f"{int(count * timeframe_sec / 60) + 30}m"
            ...
            return yf_df[['open','high','low','close','volume']].tail(count)
        except Exception as e:
            print(f"Yahoo data error ({pair}): {e}")
            return pd.DataFrame()
...
class ProLevelSD_SR:
    ...
    def find_order_blocks(self, df, recent=10):
        ...
        for i in range(2, len(df) - 1):
            if df['close'].iloc[i] < df['open'].iloc[i] and df['high'].iloc[i + 1] > df['high'].iloc[i]:
                vol = df['volume'].iloc[i] if 'volume' in df else 1
                avg_vol = df['volume'].rolling(20).mean().iloc[i] if 'volume' in df and not df['volume'].isna().all() else 1
                strength = vol / avg_vol if avg_vol > 0 else 1
                obs.append({
                    'type': 'demand',
        ...
        for i in range(self.swing_window, len(highs) - self.swing_window):
            if highs[i] == max(highs[i - self.swing_window:i + self.swing_window + 1]):
                ...
                if touches >= min_touches:
                    resistances.append({'price': float(highs[i]), 'touches': touches, 'type': 'resistance'})
            if lows[i] == min(lows[i - self.swing_window:i + self.swing_window + 1]):
                touches = self._count_touches(lows[i], df, 'low')
                if touches >= min_touches:
                    supports.append({'price': float(lows[i]), 'touches': touches, 'type': 'support'})
    ...
    def final_indicator_check(self, df, direction):
        ...
        adx_val = adx(df)
        if direction == 'BUY':
            rsi_ok = CONFIG['RSI_OS'] < rsi_val < CONFIG['RSI_OB']
        else:
            rsi_ok = CONFIG['RSI_OS'] < rsi_val < CONFIG['RSI_OB']
    ...
    def entry_check(self, df, direction):
        ...
        # 1. Main filter - S/D zone
        zones = self.find_order_blocks(df)
        in_zone, _ = self._is_in_zone(price, zones, direction)
        ...
        near_sr, _ = self._is_near_sr(price, supports if direction == 'BUY' else resistances, direction)
    ...
    def _is_near_sr(self, price, levels, direction, threshold=None):
        if threshold is None:
            threshold = CONFIG['SR_PROXIMITY']
        for lvl in levels:
...
# --- 9-Filter Gate (all mandatory, S/D relaxed) ---
pro_sd_sr = ProLevelSD_SR()

def filter_gate(df1, df3, df5, direction, platform, real_df=None):
    checks = {}

    # 1. Market structure (gate filter)
    ...
    # 6. S/D + S/R + Final Confirmation (layered filter)
    #    S/R confirmed and indicator_final are mandatory;
    #    sd_zone is checked but NOT mandatory (relaxed for OTC Blitz)
    ...
    # sr_confirmed and indicator_final must pass; sd_zone is bonus
    if not (checks['sr_confirmed'] and checks['indicator_final']):
        return False, checks

    # Cross-validation with real forex data
    if real_df is not None and not real_df.empty:
        real_structure = market_structure(real_df)
        if direction == 'BUY' and real_structure not in ['UP_TREND', 'RANGING']:
            return False, checks
        if direction == 'SELL' and real_structure not in ['DOWN_TREND', 'RANGING']:
            return False, checks
    ...
    # 9. Smart Money (already covered by S/D, kept for clarity)
    ...
    # 9 mandatory filters (sd_zone removed - relaxed for OTC Blitz)
    mandatory = ['adx', 'mtf', 'news', 'candle', 'sr_confirmed', 'indicator_final', 'liquidity', 'volume', 'momentum']
...
class UltimateEngine:
    def generate(self, market_data, pair, platform, timeframe, real_market_df=None):
        ...
        passed, checks = filter_gate(df1, df3, df5, direction, platform, real_market_df)
        if not passed:
            opp = 'SELL' if direction == 'BUY' else 'BUY'
            passed2, checks2 = filter_gate(df1, df3, df5, opp, platform, real_market_df)
            if passed2:
        ...
        lines = [
            ...
            "\u2500\u2500 \U0001f6e1\ufe0f MARTINGALE RECOVERY \u2500\u2500"
        ]
        for m in martingale:
        ...
        lines += [
            ...
            "\U0001f4a1 Trade 1% - 3% of your account",
            "\u2501" * 24
        ]
        return {
            'formatted_signal': '\n'.join(lines),
...
# --- Session Pairs (with Asian breakout) ---
def select_best_pairs():
    hour = datetime.now(pytz.UTC).hour
    if 1 <= hour < 3:      # Asian breakout (Tokyo open)
        return ['AUDJPY-OTC', 'NZDJPY-OTC', 'AUDUSD-OTC']
    if 8 <= hour < 10:     # London
        return ['EURUSD-OTC', 'GBPUSD-OTC', 'EURGBP-OTC']
    if 13 <= hour < 17:    # London/NY overlap & NY
        return ['XAUUSD-OTC', 'EURJPY-OTC', 'USDCAD-OTC', 'BTCUSD-OTC', 'USDBRL-OTC']
...
async def signal(platform: str = "iq", pair: str = None, timeframe: str = "1m"):
    ...
    for p in pairs:
        try:
            df1 = feed.get_otc_candles(platform, p, tf_sec[timeframe], 200)
            df3 = feed.get_otc_candles(platform, p, tf_sec[timeframe] * 3, 200)
            df5 = feed.get_otc_candles(platform, p, tf_sec[timeframe] * 5, 200)
            if df1.empty:
                continue
            # Cross-validate with real forex data
            forex_df = feed.get_forex_candles(p, tf_sec[timeframe], 200)
            sig = engine.generate({'1m': df1, '3m': df3, '5m': df5}, p, platform, timeframe, forex_df)
